  ...
  "The GameLogic decides if the Player Aktion is with in the Rules" when {
    "A Logic is in place" should {
      ...
      "when a Card is played wrongly" in {
        ...
        logic.applyTryLogic(field,2,2,playerBot,11) should be(INPUTFAIL)
        logic.applyTryLogic(field,4,1,playerBot,0) should be(INPUTFAIL)
        logic.applyTryLogic(field,3,3,playerBot,0) should be(INPUTFAIL)
