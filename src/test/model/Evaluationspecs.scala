...
class Evaluationspecs extends WordSpec with Matchers {
  "Evaluation compares the Attack Values of both Players and prints out the Winner of the Round" when{
    "Evaluation" should {
      val field = Field(4,4)
      "have a draw" in {
        Evaluation().eval(field,Player("Stefan", HandCard(Vector[Card]())) ,Player("Adrian",HandCard(Vector[Card]()))) should be (0)
      }
      "have playerTop win" in {
        //Player("Stefan").hand.playCard(0, field, 0, 0)
        field.set(0,0,Card("Test",1,1,1))
        Evaluation().eval(field, Player("Stefan",HandCard(Vector[Card]())), Player("Adrian",HandCard(Vector[Card]()))) should be(1)
      }
